    ...
    <div class="card card-body mb-3 mb-lg-5">
        <div class="row col-lg-divider gx-lg-6">
            <div class="col-lg-3">
                <div class="d-flex">
                    <div class="flex-grow-1">
                        <h6 class="card-subtitle mb-3">Total Pemasukan</h6>
                        <h3 class="card-title">{{number_format($payments->sum('payment_cash') + $payments->sum('payment_transfer'))}}</h3>
                    </div>
            ...
            <div class="col-lg-3">
                <div class="d-flex">
                    <div class="flex-grow-1">
                        <h6 class="card-subtitle mb-3">Transfer</h6>
                        <h3 class="card-title">{{number_format($payments->sum('payment_transfer'))}}</h3>
                    </div>
            ...
            <div class="col-lg-3">
                <div class="d-flex">
                    <div class="flex-grow-1">
                        <h6 class="card-subtitle mb-3">Cash</h6>
                        <h3 class="card-title">{{number_format($payments->sum('payment_cash'))}}</h3>
                    </div>
            ...
            <table id="datatable"
                ...
                <tbody>
                    @forelse ($payments as $payment)
                        <tr>
                            <td>{{ $payment->tanggal_transaksi->format('d-m-Y') }}</td>
                            <td>{{ $payment->order->code }}</td>
                            <td>{{ $payment->order->membership->nama }}</td>
                            <td class="text-end">{{ number_format($payment->payment_transfer) }}</td>
                            <td class="text-end">{{ number_format($payment->payment_cash) }}</td>
                            <td class="text-end">{{ number_format($payment->payment_cash + $payment->payment_transfer) }}</td>
                            <td>
                                @if ($payment->lampiran)
                                    <a href="{{ asset($payment->lampiran) }}"> <i class="bi-paperclip text-muted"></i> </a>
                                @else
                                -
                                @endif
                            </td>
                            {{-- <td class="text-end">
                                <div class="btn-group" role="group">
                                    <a class="btn btn-white btn-sm" href="{{ route('admin.order.edit', $payment) }}">
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end mt-1"
                                            ...
                                            <form action="{{ route('admin.order.destroy', $payment->id) }}" method="post"
