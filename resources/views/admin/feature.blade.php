...
@section('content')
    <x-admin-page-header
        title='Product Feature <span class="badge bg-soft-dark text-dark ms-2">{{ number_format($features->count()) }}</span>'
        subtitle="Daftar semua produk">
        <a class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#modalAddFeature">Tambah produk</a>
                        ...
                        <tr>
                            <td>
                                <a class="d-flex align-items-center"
                                    href="{{ route('admin.product.show', $feature->product) }}">
                                    <div class="flex-shrink-0">
                                        <img class="avatar avatar-lg" src="{{ asset($feature->image_url) }}"
                                            alt="Image Description">
                                    </div>
                                    <div class="flex-grow-1 ms-3">
                                        <h5 class="text-inherit mb-0">{{ $feature->product->nama }}</h5>
                                    </div>
                                </a>
                            </td>
                            <td>{{ $feature->product->kategori->nama }}</td>
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end mt-1"
                                            ...
                                            <form action="{{ route('admin.feature.destroy', $feature) }}" method="post"
                                                onsubmit="return confirm('Anda yakin ingin menghapus {{ $feature->product->nama }}?')">
                                                @csrf @method('delete')
...
@section('modal')
    <x-admin-modal id="modalAddFeature" title="Tambah Product Feature">
        <form action="{{ route('admin.feature.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label for="productLabel" class="form-label">Pilih produk</label>
                <div class="tom-select-custom">
                    <select id="productLabel" class="js-select form-select" name="product_id" autocomplete="off"
                        data-hs-tom-select-options='{
                    "searchInDropdown": false,
            ...
            <div class="mb-4">
                <label for="imageLabel" class="form-label">Upload Image</label>

                <input type="file" class="form-control" name="image_url" id="imageLabel"  required>
            </div>
