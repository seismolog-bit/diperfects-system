            ...
            <table id="datatable"
                ...
                <tbody>
                    @forelse ($orders as $order)
                        <tr>
                            <td>
                                <a class="d-flex align-items-center" href="{{ route('admin.order.show', $order) }}">
                                    {{-- <div class="flex-shrink-0">
                                        <img class="avatar avatar-lg" src="{{ asset($order->image_url) }}"
                                            alt="Image Description">
                                    </div> --}}
                                    <div class="flex-grow-1">
                                        <h5 class="text-inherit mb-0">{{ $order->nama }}</h5>
                                    </div>
                                </a>
                            </td>
                            <td>{{ $order->order_type->nama }}</td>
                            <td>{{ $order->nomor_hp }}</td>
                            <td>{{ $order->orders->count() }}</td>
                            <td>{{ ucwords(strtolower($order->kecamatan->name . ', ' . $order->kabupaten->name . ', ' . $order->provinsi->name)) }}</td>
                            <td class="text-end">
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end mt-1"
                                            ...
                                            <form action="{{ route('admin.order.destroy', $order->id) }}" method="post"
                                                onsubmit="return confirm('Anda yakin ingin menghapus {{ $order->nama }}?')">
                                                @csrf @method('delete')
